...
## 1- What is a 4-Way Set Associative Cache?
A set-associative cache allows each memory block to map to a specific set containing multiple lines (ways).

In 4-way set associative, each set contains 4 lines. The cache index selects a set, but within that set, two tags are compared in parallel. Replacement policy (e.g., LRU) decides which line to evict when the set is full.
